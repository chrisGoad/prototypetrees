...
const normalizeAngle = function (a) {
...
const movetoInGlobalCoords = function (nd, x, y) {
  // only for points for now; inputs are in global coordinates
  let p = toPoint(x, y);
...
LineSegment.intersect = function (line1) {
  ...
  if (t < 0 || t > length + 0) {
    // line1 terminates before it meets line0
    return false;
  }
...
Rectangle.scaleCentered = function (sc) {
  // while maintaining the same center
  let wd = this.width();
  let ht = this.height();
  let cnt = this.center();
  let swd = sc * wd;
...
Rectangle.applyTransform = function (tr) {
  ...
  if (rt === 0) {
    ...
    xcorners = corners.map(function (c) {
      return c.applyTransform(tr);
    });
...
const degreesToRadians = function (n) {
  return Math.PI * (n / 180);
};

const radiansToDegrees = function (n) {
...
export { movetoInGlobalCoords, toOwnCoords, toPoint, angleToDirection, Point, Rectangle, Transform, LineSegment, boundsForRectangles };
