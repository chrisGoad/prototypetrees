...
const serialize = function (node) {
  ...
  beforeSerialize.forEach(function (fn) {
    fn(node);
  });
  let rs = encode(node);
  node.__sourceUrl = srcp;
  afterSerialize.forEach(function (fn) {
    fn(node);
  });
  return rs;
};
...
const propsToIgnore = ['__code', '__element', '__container', '__setCount', '__setIndex', '__notHead', '__parent', '__label', '__resizeBoxes', '__selected', '__updateCount', '__beenAdjusted'];
...
const collectNodes = function (n1, n2) {
  // traverse the trees in order given by the ownprops of n1
  let nodeMap = [];
  let labelCount = 1;

  const collectNodesR = function (n1, n2, callDepth = 0) {
    // traverse the trees in order given by the ownprops of n1
...
const saveState = function () {
  ...
  beforeSaveStateHooks.forEach(fn => {
    fn();
  });
...
const undo = function () {
  ...
  if (isDiff(current)) {
    ...
    // we have moved back to an old state with a different structure
