...
const parseWithDOM = function (s, forXML) {
  ...
  if (!dm || !dm.firstChild || dm.firstChild.tagName === "html") {
    // an error message
...
core.ObjectNode.__iterDomTree = function (fn) {
  ...
  ownprops.forEach(function (k) {
    let ch;
    if (core.treeProperty(thisHere, k, true, true)) {
...
core.setChildHooks.push(function (node, nm, c) {
  ...
  if (__isDomEl(node)) {
    ...
    if (nm === "transform" && Transform.isPrototypeOf(c)) {
      //special treatment for transforms
...
Element.__rootElement = function () {
  // find the most distant ancestor which is an Element
  let cv = this;
  let nv;
  while (true) {
    nv = cv.__parent;
...
function getJXONTree(oXMLParent, forXML) {
  ...
  if (oXMLParent.attributes) {
    // cg added the check for existence of method
...
SvgElement.mk = function () {
  return Object.create(SvgElement);
};
...
SvgElement.bounds = function (rt) {
  ...
  if (el) {
    ...
    if (this !== core.root && this.hasHiddenDescendant()) {
      // bbox includes hidden parts, but we do not wish to include them, so have to recurse
...
const showRectangle = function (bnds) {
  ...
  core.root.set(nm, SvgElement.mk('<rect x="' + crn.x + '" y="' + crn.y + '" width="' + ext.x + '" height="' + ext.y + '" stroke="red" stroke-width="2" fill="transparent"/>'));
};
...
const highlightExtraNode = function (node) {
  ...
  if (extraNodeHighlighted) {
    if (extraNodeHighlighted === node) {
      ...
      // replace the node begin highlighted
...
const highlightNodes = function (inodes) {
  ...
  let nodes = inodes.filter(function (node) {
    return node.__hidden && !node.__hidden();
  });
...
tag.tspan.mk = function () {
  return Object.create(tag.tspan);
};
...
svg.stdColors = ["rgb(100,100,200)", "rgb(100,200,100)", "red", "yellow", "red", "rgb(244,105,33)", "rgb(99,203,154)", "rgb(207,121,0)", "rgb(209,224,58)", "rgb(51, 97, 204)", "rgb(216,40,165)", "rgb(109,244,128)", "rgb(77,134,9)", "rgb(1,219,43)", "rgb(182,52,141)", "rgb(48,202,20)", "rgb(191,236,152)", "rgb(112,186,127)", "rgb(45,157,87)", "rgb(80,205,24)", "rgb(250,172,121)", "rgb(200,109,204)", "rgb(125,10,91)", "rgb(8,188,123)", "rgb(82,108,214)"];
...
core.ObjectNode.__updateVisibleInheritors = function () {
  core.updateInheritors(this, function (x) {
    x.__update();
  }, isVisible);
};
...
const updateVisibleParts = function (node) {
  core.updateParts(node, function (part) {
...
export { SvgRoot, SvgElement, tag as SvgTag, setSvgMain, svgMain, unhighlight, svg, highlightNodes, highlightExtraNode, centerOnOrigin, fullUpdate, stdTransferredProperties }; // Copyright 2019 Chris Goad
// License: MIT
...
core.beforeStringify.push(function (itm) {
  stashPreSave(itm, 1);
});
